...
# Get the server settings from the config.toml file
enable_xsrf = config.get('server', {}).get('enableXsrfProtection', False)
enable_cors = config.get('server', {}).get('enableCORS', False)
...
# Add a header
st.header("Please enter your OpenAI API Key")

# Check if the user has already entered the key
if "OPENAI_API_KEY" not in st.session_state:
    # Ask the user to input their API key
    api_key = st.text_input("Enter OpenAI API Key", type="password")
    
    if api_key:
        # Store the key in session state and environment
        st.session_state["OPENAI_API_KEY"] = api_key
        os.environ["OPENAI_API_KEY"] = api_key
        openai.api_key = api_key
        st.success("API Key has been set!")
else:
    # If key is already set, confirm it
    st.success("API Key is already set.")

# Optionally, allow the user to clear the API key
